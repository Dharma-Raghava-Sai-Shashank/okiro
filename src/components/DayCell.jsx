...
export default function DayCell({
  ...
  const primaryTint =
    dayTasks.length > 0 ? dayTasks[0].color || "#ede9fe" : null;
  ...
  const renderGlassCards = () => {
    ...
    return (
      <div className="no-scrollbar flex flex-col gap-[1px] w-full overflow-y-auto z-[1] pb-0.5" style={{ maxHeight: "100px" }}>
        {dayTasks.map((t) => (
          <GlassyTaskCard
            key={t._id}
            t={t}
      ...
      {total > 0 && !useGlassCards && (
        <div className="mt-1.5 flex items-center gap-1.5 relative z-[1]">
          ...
          <span
            ...
            style={{
              color:
                done === total ? "rgb(5, 150, 105)" : "rgba(15, 23, 42, 0.55)",
            }}
          >
            {done}/{total}
          </span>
...
function GlassyTaskCard({ t, dayKey, onOpen, onMouseEnter, onMouseLeave }) {
  ...
  const FLAME_PATH =
    "M12 2c0 2 1 3.6 2 5.2c1.7 2.6 4 4.8 4 8.3a6 6 0 0 1-12 0c0-2.7 1-4.6 2-6.2c.8 1.7 1.7 1.7 1.7-.4c0-2 1.1-4.5 2.3-6.9z";
  const INNER_FLAME_PATH =
    "M12 11c0 1 .5 1.7 1 2.5c.8 1.2 1.6 2.1 1.6 3.5a2.6 2.6 0 0 1-5.2 0c0-1.2.4-2 .9-2.8c.4.7.7.7.7-.2c0-.9.4-2 1-3z";
  ...
  return (
    ...
    >
      ...
      >
        <svg
          viewBox="0 0 24 24"
          className="w-[105%] h-[105%] transition-transform duration-300 ease-out group-hover/card:scale-[1.7] group-hover/card:-translate-y-1 overflow-visible"
        >
          <path
            d={FLAME_PATH}
            fill={deep}
